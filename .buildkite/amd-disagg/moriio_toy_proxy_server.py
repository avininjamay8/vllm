...
async def start_decode_request(endpoint, req_data, request_id, selected_decode_dp_rank=None):
...
async def handle_request(api: str, request: Request):
    try:
        ...
        # Select decode DP rank here so we can tell prefill which rank to target
        selected_decode_dp_rank = None
        if decode_instance_endpoint["dp_size"] > 1:
            selected_decode_dp_rank = request_nums % decode_instance_endpoint["dp_size"]
        ...
        # In READ mode prefill generates 1 token and decode generates the
        # remaining max_tokens-1. In WRITE mode decode generates all tokens;
        # subtracting 1 here would break requests with max_tokens=1.
        ...
        # each send a notify, flooding prefill with empty block_notify_lists.
        req_data["kv_transfer_params"]["is_request_leader"] = True
        # Explicit prefill address fields: used as fallback when the internal
        # vLLM request_id is a short hex (not our full ___prefill_addr_... format)
        # so that _release_write_prefill_blocks can find the notify address.
        _prefill_zmq = prefill_instance_endpoint["zmq_address"]
        _prefill_host = _prefill_zmq.split(",")[0].split(":")[-1]
        _prefill_notify = _prefill_zmq.split("notify:")[-1]
        req_data["kv_transfer_params"]["remote_host"] = _prefill_host
        req_data["kv_transfer_params"]["remote_notify_port"] = int(_prefill_notify)

        decode_request_url = decode_instance_endpoint["request_address"] + api
        # remote_dp_rank for decode already set above to selected_prefill_dp_rank
        # (tells decode which prefill DP rank to notify "blocks ready")
        # selected_decode_dp_rank is passed via X-data-parallel-rank header only

        decode_request_task = asyncio.create_task(
            start_decode_request(decode_request_url, req_data, request_id, selected_decode_dp_rank)
        )
